...
/*!**********************************************
 * \fn void MakeSPECalibrationHists::MakeAllHists(const std::vector<std::shared_ptr<SPECalibrationVariables>> & cal, std::string pathPrefix, std::string pdfVar, int nEntries, bool fillAllHists, bool peHistsFlag, bool rate2DHistFlag, bool spe2DHistFlag, bool adc2DHistFlag, bool flangeHistsFlag, bool spe1DHistFlag, bool rate1DHistFlag, bool noisePeakHistFlag, bool noiseEndHistFlag, bool fitEndHistFlag, bool fitRMSHistFlag, bool noiseIntegralHistFlag, bool tailIntegralHistFlag, bool rootOnlyFlag, bool pdfOnlyFlag, bool rootAndPDFFlag, bool rootTreeFlag)
 ...
 * \param[in] fillAllHists Flag to fill all the hists
 ...
 ***********************************************/
void MakeSPECalibrationHists::MakeAllHists(const std::vector<std::shared_ptr<SPECalibrationVariables>> & cal, std::string pathPrefix,
    std::string pdfVar, int nEntries, bool fillAllHists, bool peHistsFlag, bool rate2DHistFlag, bool spe2DHistFlag, bool adc2DHistFlag, 
    bool flangeHistsFlag, bool spe1DHistFlag, bool rate1DHistFlag, bool noisePeakHistFlag, bool noiseEndHistFlag, bool fitEndHistFlag,
    bool fitRMSHistFlag, bool noiseIntegralHistFlag, bool tailIntegralHistFlag, bool rootOnlyFlag, bool pdfOnlyFlag, bool rootAndPDFFlag, bool rootTreeFlag)
{

  SetSizeOfCal(cal.size());
  HistInfo_t myEnum;
  if(rootOnlyFlag == true || rootAndPDFFlag == true || rootTreeFlag == true){
    TFile * outfile = TFile::Open(std::string(pathPrefix + "root_out_" + pdfVar + "_.root").c_str(),"RECREATE");
    outfile->cd();
  }

  if(rate2DHistFlag == true){
    myEnum = kRate2DHist;
    fHistInfoVec.push_back(myEnum);
  }
  if(spe2DHistFlag == true){
    myEnum = kSPE2DHist;
    fHistInfoVec.push_back(myEnum);
  }
  if(adc2DHistFlag == true){
    myEnum = kADC2DHist;
    fHistInfoVec.push_back(myEnum);
  }
  if(flangeHistsFlag == true){
    myEnum = kFlangeHists1;
    fHistInfoVec.push_back(myEnum);
    myEnum = kFlangeHists2;
    fHistInfoVec.push_back(myEnum);
  }
  if(spe1DHistFlag == true){
    myEnum = kSPE1DHist;
    fHistInfoVec.push_back(myEnum);
  }
  if(rate1DHistFlag == true){
    myEnum = kRate1DHist;
    fHistInfoVec.push_back(myEnum);
  }
  if(noisePeakHistFlag == true){
    myEnum = kNoisePeakHist;
    fHistInfoVec.push_back(myEnum);
  }
  if(noiseEndHistFlag == true){
    myEnum = kNoiseEndHist;
    fHistInfoVec.push_back(myEnum);
  }
  if(fitEndHistFlag == true){
    myEnum = kFitEndHist;
    fHistInfoVec.push_back(myEnum);
  }
  if(fitRMSHistFlag == true){
    myEnum = kFitRMSHist;
    fHistInfoVec.push_back(myEnum);
  }
  if(noiseIntegralHistFlag == true){
    myEnum = kNoiseIntegralHist;
    fHistInfoVec.push_back(myEnum);
  }
  if(tailIntegralHistFlag == true){
...
void MakeSPECalibrationHists::FillFlangeHists(const std::vector<std::shared_ptr<SPECalibrationVariables>> & cal, bool rootOnlyFlag, bool rootAndPDFFlag)
{
  ...
  for (int p = 0; p < fSizeOfCal; ++p) {
    ...
    bool isVeto = cal.at(p)->GetIsVeto();
    double x = 0;
    double y = 0;
