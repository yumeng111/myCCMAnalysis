...
void Usage() 
{
  ...
  (*usage) << "Usage: FindEvents [options] \n"
    << "options are:\n"
    << "  -i file.bin           : Add input data file\n"
    << "  -o file.root          : Set output data file\n"
    << "  -t trigger type       : Set trigger type name (beam, strobe, LED)\n";
...
int main(int argc, char ** argv)
{
  ...
  while (1) {
    ...
    switch (c) {
      ...
      case kTriggerTypeOpt:   triggerType = std::string(optarg); break;
  ...
  std::ifstream hvOffList("hv_off_2019.csv");
  int board = 0;
  int chan = 0;
  while (hvOffList >> board >> chan) {
    int key = PMTInfoMap::ConvertHVBoardChanToKey(board,chan);
    ...
    highVoltage.insert(std::make_pair(key,HighVoltage_t(0,0)));
  }
  hvOffList.close();
  ...
  TTreeReader calibrationTreeTop("spe",calibrationFileTop);
  ...
  TTreeReaderValue<float> speValueTop(calibrationTreeTop,"speValue");
  TTreeReaderValue<float> speThresholdTop(calibrationTreeTop,"endNoiseWallFitRangeStart");
  TTreeReaderValue<int> pmtIDTop(calibrationTreeTop,"pmtID");
  while (calibrationTree.Next()) {
    calibrationValues.insert(std::make_pair(*pmtID,std::make_pair(*speValue,*speThreshold)));
  }
  while (calibrationTreeTop.Next()) {
    calibrationValuesTop.insert(std::make_pair(*pmtIDTop,std::make_pair(*speValueTop,*speThresholdTop)));
  }
  delete calibrationFile;
  delete calibrationFileTop;

  for (int i=0; i < 160; ++i) {
    std::map<int,std::pair<float,float>>::iterator it = calibrationValues.find(i);
    std::map<int,std::pair<float,float>>::iterator itTop = calibrationValuesTop.find(i);

    if (it == calibrationValues.end() || itTop == calibrationValuesTop.end()) {
      continue;
    }

    double bot = std::min(40.f,it->second.first);
    double top = std::min(40.f,itTop->second.first);

    //it->second.first = (top+bot)/2.0;
    it->second.first = top;
    std::map<int,HighVoltage_t>::iterator itHigh = highVoltage.find(i);
    if (itHigh == highVoltage.end()) {
      continue;
    }

    if (itHigh->second.status == 0 || itHigh->second.power == 0) {
      it->second.first = -1;
    }
  ...
  // Setup output files
  std::shared_ptr<TFile> outfile = std::make_shared<TFile>(Form("%s",outFileName.c_str()),"RECREATE");
  ...
  // various triggers that equate to the \p triggerType
  ...
  for (e = 0; e < nEntries; ++e) {
    ...
    int firstSample = 0;
    if (triggerType.find("strobe") != std::string::npos) {
      firstSample = Utility::FindFirstSample(boardOffset+1,rawData);
      if (!firstSample) {
        ++skipped;
        continue;
      }
    } else if (triggerType.find("beam") != std::string::npos) {
      firstSample = Utility::FindFirstSample(boardOffset+2,rawData);
      if (!firstSample) {
        ++skipped;
        continue;
      }
    } else if (triggerType.find("led") != std::string::npos) {
      ...
      MsgWarning(MsgLog::Form("Trigger type is %s and does not equal strobe, beam, or led. Skipping Event",triggerType.c_str()));
      continue;
    }
    ...
    for (size_t loc = 0; loc < kNumPulses; ++loc) {
      ...
      double pe = calibrationValues[key].first;
      ...
      if (integralTime.at(bin) >= 0.4) {
        ...
        if (bin >= 0 && bin < pmtWaveform.front().size()) {
          for (int i=0; i < 160; ++i) {
            ...
            double pe = calibrationValues[i].first;//pmtInfo->GetADCToPE();
            if (pe <= 0) {
              continue;
            }
            if (pmtInfo->IsUncoated()) {
