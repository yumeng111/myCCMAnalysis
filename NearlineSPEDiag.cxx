...
void NearlineSPEDiag::CreatePEHists()
{
  ...
  //}
	for(int i=0; i<fSizeOfCal; ++i){
		...
		fCal.at(i)->SetPEHistPtr(std::make_shared<TH1D>(Form("peHist%d",i),";ADC;Count Above Background",2000,0,1000));
		fCal.at(i)->SetPMTID(i);
		fCalBefore.at(i)->SetPEHistPtr(std::make_shared<TH1D>(Form("peHistBefore%d",i),";PE;Count",2000,0,1000));
		fCalBefore.at(i)->SetPMTID(i);
	}
}

/*!**********************************************
 * \fn void NearlineSPEDiag::MakeChainFillPulses(const std::vector<std::string> & fileList, bool ledRunFlag, int numLEDTriggers)
 ...
 * could come from any trigger. The flag also determines the DAQ window to look for events. Currently if
 * - \p ledRunFlag = false #fWindowStart = -9.92 \f$\mu\f$s and #fWindowEnd = -1 \f$\mu\f$s,
 * - \p ledRunFlag = true #fWindowStart = 0.25 \f$\mu\f$s and #fWindowEnd = 0.35 \f$\mu\f$s.
...
void NearlineSPEDiag::MakeChainFillPulses(const std::vector<std::string> & fileList, bool ledRunFlag)
{
  ...
  if(ledRunFlag == true){
    //fWindowStart = -0.725;
    //fWindowEnd = -0.625;
    //fWindowStart = 0.13; // bottom led
    //fWindowEnd = 0.28; // bottom led
    fWindowStart = 0.25; // top led
    fWindowEnd = 0.35; // top led
  }else{
    fWindowStart = -9.92;//start of trigger fWindow to look at
    fWindowEnd = -1.0;//end of trigger fWindow to look at
  }
  ...
  for (long e = 0; e < fNEntries-1; ++e) {
    ...
    int numPulses = pulses->GetNumPulses();
    //pulses->Sort();  
 ...
 ***********************************************/
void NearlineSPEDiag::GetHistsToAdjust(std::string fileToAdjust, bool ledRunFlag)
{ 
	if(ledRunFlag == true){
   	//fWindowStart = 0.13; // bottom led
   	//fWindowEnd = 0.28; // bottom led
...
/*!**********************************************
 * \fn void NearlineSPEDiag::FillHist(std::string pathPrefix, std::string pdfVar,bool fillAllHistFlag,bool rate2DHistFlag,bool spe2DHistFlag,bool peHistsFlag, bool adc2DHistFlag,bool flangeHistsFlag,bool spe1DHistFlag,bool rate1DHistFlag,bool noisePeakHistFlag,bool noiseEndHistFlag, bool fitEndHistFlag,bool fitRMSHistFlag,bool noiseIntegralHistFlag,bool tailIntegralHistFlag,bool rootOnlyFlag, bool pdfOnlyFlag, bool rootAndPDFFlag,bool rootTreeFlag)
 ...
 * \param[in] fillAllHistFlag Flag to fill all the hists
 * \param[in] peHistsFlag Flag to fill the PE hists
 * \param[in] rate2DHistFlag Flag to fill the 2D rate hists
 * \param[in] spe2DHistFlag Flag to fill the 2D SPE hists
 * \param[in] adc2DHistFlag Flag to fill the 2D ADC hists
 * \param[in] flangeHistsFlag Flag to fill the flange hists
 * \param[in] spe1DHistFlag Flag to fill the 1D SPE hist
 * \param[in] rate1DHistFlag Flag to fill the 1D rate hist
 * \param[in] noisePeakHistFlag Flag to fill the noise peak hist
 * \param[in] noiseEndHistFlag Flag to fill the noise end hist
 * \param[in] fitEndHistFlag Flag to fill the fit end hist
 * \param[in] fitRMSHistFlag Flag to fill the fit RMS hist
 * \param[in] noiseIntegralHistFlag  Flag to fill the noise integral hist
 * \param[in] tailIntegralHistFlag Flag to fill the tail integral hist
 * \param[in] rootOnlyFlag Flag to save the hists only in the ROOT file
 * \param[in] pdfOnlyFlag Flag to save the hists only to a PDF
 * \param[in] rootAndPDFFlag Flag to save the hists in a ROOT file and in a pdf
 * \param[in] rootTreeFlag Flag to save the calibration values as a Tree in the ROOT file
 ***********************************************/
void NearlineSPEDiag::FillHist(std::string pathPrefix, std::string pdfVar,bool fillAllHistFlag,bool rate2DHistFlag,bool spe2DHistFlag,bool peHistsFlag,
    ...
    bool rootAndPDFFlag,bool rootTreeFlag)
{
  ...
  MakeSPECalibrationHists::MakeAllHists(fCal,pathPrefix,pdfVar,fNEntries,fillAllHistFlag,peHistsFlag,
      rate2DHistFlag,spe2DHistFlag, adc2DHistFlag, flangeHistsFlag, spe1DHistFlag, rate1DHistFlag, 
      noisePeakHistFlag,noiseEndHistFlag, fitEndHistFlag,fitRMSHistFlag,noiseIntegralHistFlag,
      tailIntegralHistFlag,rootOnlyFlag,pdfOnlyFlag, rootAndPDFFlag,rootTreeFlag);
