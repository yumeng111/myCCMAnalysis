...
CCMResult_t CCMFindEvents::ProcessTrigger()
{
  ...
  for (size_t timeBin = 0; timeBin < gkMaxBinLoc && timeBin < Utility::fgkNumBins; ++timeBin) {
    ...
    //if (MsgLog::GetGlobalLogLevel() >= 2 && fAccumWaveformMethodID == kCCMAccumWaveformTrianglePulseCutID) {
...
void CCMFindEvents::SaveEvent(int startBin, int endBin)
{
  ...
  double startTime = Utility::ShiftTime(startBin,fAccumWaveform->GetBeamOffset());
  double endTime = Utility::ShiftTime(endBin,fAccumWaveform->GetBeamOffset());
  ...
  promptIntegral = fAccumWaveform->Integrate(startBin,endBin,fAccumWaveformMethodID,kCCMIntegralTimeID);
