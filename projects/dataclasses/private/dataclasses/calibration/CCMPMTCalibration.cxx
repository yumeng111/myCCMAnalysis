...
double
CCMPMTCalibration::GetATWDBinCalibSlope(unsigned int id,
...
void
CCMPMTCalibration::SetATWDBinCalibSlope(unsigned int id,
                                       unsigned int channel,
                                       unsigned int bin,
                                       double slope){
  assert(id<2);
...
QuadraticFit
CCMPMTCalibration::GetATWDFreqFit (unsigned int chip) const
{
  ...
  if(chip == 0 || chip ==1 )
    ...
    {
      log_fatal("Frequency calib not found for ATWD chip in CCMPMTCalibration");
    }
}

void
CCMPMTCalibration::SetATWDFreqFit (unsigned int chip,
          QuadraticFit fitParams)
{
  if(chip == 0 || chip ==1 )
    ...
    {
      log_fatal("Invalid chip specified in CCMPMTCalibration::SetATWDFreqFit");
    }
}

void
CCMPMTCalibration::SetATWDGain (unsigned int channel,
             double gain)
{
  if(channel == 0 || channel == 1 || channel == 2)
    ...
    {
      log_fatal("Invalid channel specified in CCMPMTCalibration::SetATWDGain");
    }
}

double
CCMPMTCalibration::GetATWDGain (unsigned int channel) const
{
  if(channel == 0 || channel == 1 || channel == 2)
    {
    ...
    {
      log_fatal("Invalid ATWD channel requested CCMPMTCalibration::GetATWDGain");
    }
}

void
CCMPMTCalibration::SetATWDDeltaT (unsigned int chip,
                                 double deltat)
{
  if(chip == 0 || chip == 1)
    {
    ...
    {
      log_fatal("Invalid chip id specified in CCMPMTCalibration::SetATWDDeltaT");
    }
}

double
CCMPMTCalibration::GetATWDDeltaT (unsigned int chip) const
{
  if(chip == 0 || chip == 1)
    {
    ...
    {
      log_fatal("Invalid chip id requested CCMPMTCalibration::GetATWDDeltaT");
    }
}

double
CCMPMTCalibration::GetATWDBeaconBaseline(unsigned int id, unsigned int channel) const
{
  if ((id == 0 || id ==1) &&
...
void
CCMPMTCalibration::SetATWDBeaconBaseline(unsigned int id, unsigned int channel, double bsl)
{
  if ((id == 0 || id ==1) &&
      ( channel == 0 || channel == 1 || channel == 2)) {
...
CCMPMTCalibration::DiscriminatorPulseTemplate(bool droopy) const
{
...
CCMPMTCalibration::DroopedSPETemplate
CCMPMTCalibration::ATWDPulseTemplate(unsigned channel, bool droopy) const
{
  if (channel > 2)
...
CCMPMTCalibration::DroopedSPETemplate
CCMPMTCalibration::FADCPulseTemplate(bool droopy) const
{
  if(!droopy)
...
CCMPMTCalibration::serialize(Archive& ar, unsigned version)
{
  if (version>i3domcalibration_version_)
    log_fatal("Attempting to read version %u from file but running version %u of CCMPMTCalibration class.",version,i3domcalibration_version_);
  ...
  if(version < 4)
    {
      ...
      while(iter != ampGains_temp.end())
      {
        if(iter->first < 3)
          ...
          {
            log_fatal("Invalid channel for ampGains in CCMPMTCalibration::serialize");
          }
              iter++;
      }
    }
  ...
  if(version < 4)
    {
      ...
      while(iter !=  atwdFreq_temp.end())
      {
        if(iter->first < 2)
          ...
          {
            log_fatal("Invalid chip number for atwdFreq in CCMPMTCalibration::serialize");
          }
              iter++;
      }
    }
      ...
      while(iter_nch != atwdBin0_temp.end())
      {
            ...
            while(iter_nbin != iter_nch->second.end() )
        {
          if(iter_nbin->first<128)
            ...
            {
              log_fatal("Invalid bin for atwdBin_ in CCMPMTCalibration::serialize");
            }
                      iter_nbin++;
        }
          }
          ...
          {
            log_fatal("Invalid channel for atwdBin_ in CCMPMTCalibration::serialize");
          }
              iter_nch++;
      }
      //  For atwdBin1:
