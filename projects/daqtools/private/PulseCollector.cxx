...
std::vector<double> GetSamplesBeforeThreshold(CCMWaveform const & waveform, double deriv_threshold, double tau, double delta_t, size_t max_samples) {
    ...
    for(size_t i=1; i<N-2; ++i) {
        ...
        smoothed_wf[i+1] = (exp_current + exp_next + exp_nextnext) / 3.0;
        exp_current = exp_next;
        exp_next = exp_nextnext

        // Finite difference derivative
        derivative[i] = (smoothed_wf[i+1] - smoothed_wf[i-1]) / (2.0 * delta_t);

        if((derivative[i] > deriv_threshold) or (i == max_samples - 1)) {
            over_threshold = true;
    ...
    smoothed_wf.resize(last_index + 1);
    return smoothed_wf;
}
...
class PulseCollector : public I3Module {
    ...
    std::map<CCMPMTKey, std::deque<std::vector<double>>> ordered_baseline_samples;
    std::map<CCMPMTKey, std::multiset<double>> sorted_baseline_samples
...
void PulseCollector::RemoveBaselineSamples() {
    for(std::pair<CCMPMTKey const, std::deque<std::vector<double>>> p : ordered_baseline_samples) {
        std::deque<std::vector<double>>> & sample_history = p.second;
        if(sample_history.size() == 0) {
            continue;
        }
        std::vector<double> const & to_remove = sample_history.front();
        std::multiset<double> & sorted = sorted_baseline_samples[p.first];
...
std::vector<double> PulseCollector::SmoothWaveform(std::vector<uint16_t>::const_iterator begin, std::vector<uint16_t>::const_iterator end) {
    // Invert waveform
...
void PulseCollector::AddBaselineSamples(I3FramePtr frame) {
...
void PulseCollector::EstimateBaselines() {
...
double PulseCollector::QuickBaselineEstimate(CCMWaveformUInt16 const & waveform, double baseline, double baseline_stddev) {
...
NIMLogicPulseSeries PulseCollector::GetNIMPulses(CCMWaveformUInt16 const & waveform, double baseline, double baseline_stddev) {
...
bool PulseCollector::CheckBaselines(I3FramePtr frame) {
