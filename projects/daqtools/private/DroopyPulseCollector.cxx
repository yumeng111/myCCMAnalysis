...
class DroopyPulseCollector : public I3Module {
    ...
    std::map<CCMPMTKey, std::deque<WaveformSmoother>> smooth_waveform_cache_droop_corrected;
...
public:
    DroopyPulseCollector(const I3Context&);
    void Configure();
    void Geometry(I3FramePtr frame);
    void DAQ(I3FramePtr frame);
    void Finish();
    void Flush();
    std::vector<double> DroopCorrection(std::vector<uint16_t>::const_iterator cbegin, std::vector<uint16_t>::const_iterator cend, double baseline);
    std::pair<size_t, double> ProcessWaveform(CCMPMTKey key, CCMWaveformUInt16 const & waveform);

    void ProcessFrame(I3FramePtr frame);
};

I3_MODULE(DroopyPulseCollector);

std::pair<size_t, double> DroopyPulseCollector::ProcessWaveform(CCMPMTKey key, CCMWaveformUInt16 const & waveform) {
...
std::vector<double> DroopyPulseCollector::DroopCorrection(std::vector<uint16_t>::const_iterator cbegin, std::vector<uint16_t>::const_iterator cend, double baseline) {
    std::vector<uint16_t>::const_iterator raw_it = cbegin;
    std::vector<double> droop_corrected_raw_wf;
...
void DroopyPulseCollector::ProcessFrame(I3FramePtr frame) {
    ...
    boost::shared_ptr<I3Map<CCMPMTKey, std::vector<double>>> pulse_samples_droop_corrected = boost::make_shared<I3Map<CCMPMTKey, std::vector<double>>>();
    for(std::pair<CCMPMTKey const, uint32_t> const & p : pmt_channel_map_) {
        ...
        if(waveform.GetWaveform().size() > 0) {
            ...
            if(last_pulse_sample > 0) {
                ...
                std::vector<double> result_droop_corrected = DroopCorrection(iterators.first, iterators.second, baseline);
    ...
    frame->Put("PulseSamplesDroopCorrected", pulse_samples_droop_corrected);
}

DroopyPulseCollector::DroopyPulseCollector(const I3Context& context) : I3Module(context),
...
void DroopyPulseCollector::Configure() {
...
void DroopyPulseCollector::Geometry(I3FramePtr frame) {
...
void DroopyPulseCollector::DAQ(I3FramePtr frame) {
    if(not geo_seen) {
...
void DroopyPulseCollector::Flush() {
}

void DroopyPulseCollector::Finish() {
    Flush();
}
