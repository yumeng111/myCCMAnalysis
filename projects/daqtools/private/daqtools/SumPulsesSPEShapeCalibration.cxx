...
class SumPulsesSPEShapeCalibration : public I3Module {
    ...
    // let's save peak heights and integrals
    boost::shared_ptr<I3Map<CCMPMTKey, std::vector<double>>> pulse_integrals = boost::make_shared<I3Map<CCMPMTKey, std::vector<double>>>();
    boost::shared_ptr<I3Map<CCMPMTKey, std::vector<double>>> pulse_heights = boost::make_shared<I3Map<CCMPMTKey, std::vector<double>>>();
...
public:
    SumPulsesSPEShapeCalibration(const I3Context&);
    void Configure();
...
I3_MODULE(SumPulsesSPEShapeCalibration);

void SumPulsesSPEShapeCalibration::ProcessFrame(I3FramePtr frame) {
        ...
        for(std::pair<CCMPMTKey const, std::vector<std::vector<double>>> const & p : pulse_samples) {
            ...
            for (size_t wfs_it = 0; wfs_it < vector_of_wfs.size(); ++wfs_it){
                ...
                for (size_t wf_it = 0; wf_it < current_wf.size(); ++wf_it){
                    pulse_integral += current_wf[wf_it];
                ...
                if (max_adc_counts < max_counts_ and max_adc_counts > min_counts_){
                    ...
                    pulse_integrals->operator[](pmt_key).push_back(pulse_integral);
                    pulse_heights->operator[](pmt_key).push_back(max_adc_counts);
                }
            }
        }
    }
...
SumPulsesSPEShapeCalibration::SumPulsesSPEShapeCalibration(const I3Context& context) : I3Module(context),
...
void SumPulsesSPEShapeCalibration::Configure() {
...
void SumPulsesSPEShapeCalibration::Geometry(I3FramePtr frame) {
...
void SumPulsesSPEShapeCalibration::DAQ(I3FramePtr frame) {
    ProcessFrame(frame);
}

void SumPulsesSPEShapeCalibration::Finish() {
    ...
    frame->Put("PeakHeight" , pulse_heights);
    frame->Put("PeakIntegral" , pulse_integrals);
    PushFrame(frame);
    Flush();
}
