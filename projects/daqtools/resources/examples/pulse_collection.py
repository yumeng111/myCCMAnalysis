...
class SavePulses(I3ConditionalModule):
    ...
    def DAQ(self, frame):
        ...
        #    self.PushFrame(frame)
        #    return
        samples = frame["PulseSamples"]
        for pmt, wf in samples.items():
            output = self.output_files[pmt]
            np.savetxt(output, np.array(wf).astype(float), newline=" ")
            output.write("\n")
            self.n_pulses[pmt] += 1
