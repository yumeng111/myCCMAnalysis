...
#include <sstream>
#include "Utility.h"
#include <dataclasses/I3Matrix.h> //for ublas work-around stuff

class ublas_vector_shim :
 public boost::numeric::ublas::vector<double,ublas_storage_shim> {
private:
...
public:
  ...
  ublas_vector_shim(){}
  explicit ublas_vector_shim(size_type size):base(size){}
  ublas_vector_shim(size_type size, const ublas_storage_shim& data):
  base(size,data){}
  ublas_vector_shim(size_type size, const base::value_type& init):
  base(size,init){}
  ublas_vector_shim(const base& v):base(v){}
  template<class AE>
  ublas_vector_shim(const boost::numeric::ublas::vector_expression<AE>& ae):
  base(ae){}
};
