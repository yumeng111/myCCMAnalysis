...
namespace Laputop {
  ...
  struct Parameter {
    enum Enum {
      ...
      CurvParabA,           ///< curvature free parameters (KR adds 6/2017) 
      CurvGaussD,
      CurvGaussN,
  ...
  struct FrontDelay {
    enum Enum {
      None = -1,            ///< Indicator for I3LaputopParams to use stored value
      GaussParabola,        ///< Parabolic umbrella with a Gaussian tip around the center (IT-26 hard-coded)
      GaussParabolaEmily,   ///< Parabolic umbrella with a Gaussian tip around the center (Emily hard-coded)
      GaussParabolaFromParams ///< Parabolic umbrella with a Gaussian tip around the center (general, from Params)
    };
  };
