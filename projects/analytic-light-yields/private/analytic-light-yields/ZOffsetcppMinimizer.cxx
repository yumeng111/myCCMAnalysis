...
struct ZOffsetFitLikelihoodFunctor {
    ...
    static constexpr int NewDerivativeDimension = 3 + 1 + 2 + 4 + 200; // max number of dimensions :
                                                                       // Rs, tau_s, tau_other
    ...
    T evaluateLikelihood(std::vector<T> x) const {
        ...
        std::cout << "Rs = " << Rs.value() << ", tau_s = " << tau_s.value() << ", tau_other = " << tau_other.value() << ", norm = " << norm.value() 
            << ", uv abs 1 = " << uv_abs_1.value() << ", uv abs 2 = " << uv_abs_2.value() << ", and z offsets = " << x[6].value()
            << ", " << x[7].value() << ", " << x[8].value() << ", " << x[9].value() << std::endl;
        
        for (size_t data_it = 0; data_it < llh_constructorAD.size(); data_it ++){
            // loop over each data set we are fitting to
            T this_z_offset = x[6 + data_it];
            ...
            for (size_t pmt_it = 0; pmt_it < all_keys.size(); pmt_it ++){
                ...
                total_llh += llh_constructorAD.at(data_it)->ComputeNLLH(key, Rs, Rt, tau_s, tau_t, tau_rec, tau_other, norm,
                                time_offsets.at(data_it).at(key), const_offset, LPmu.at(key), LPsigma.at(key), LPscale.at(key), x[10 + pmt_it], // pmt eff!
                                {uv_abs_1, uv_abs_2}, photons_per_mev, this_z_offset, n_sodium_events.at(data_it), light_profile_type, use_g4_yields);
                if (use_tau_prior){
...
std::vector<double> ZOffsetcppMinimizer::GrabNormSeed(CCMPMTKey key, double baseline_efficiency, double LPmu, double LPsigma, double LPscale, 
    ...
    for (size_t data_it = 0; data_it < data_file_names.size(); data_it++){
        ...
        // now save!
        llh_constructorDouble.push_back(this_llh_constructorDouble);
    }
    ...
    for (size_t data_it = 0; data_it < llh_constructorDouble.size(); data_it ++){
        // loop over each data set we are fitting to
        double llh = llh_constructorDouble.at(data_it)->ComputeNLLH(key, Rs, Rt, tau_s, tau_t, tau_rec, tau_other, norm_seed, 
                         time_offsets.at(data_it).at(key), const_offset, LPmu, LPsigma, LPscale, baseline_efficiency,
                         uv_abs_lengths, photons_per_mev, z_offsets.at(data_it), n_sodium_events.at(data_it), light_profile_type, use_g4_yields);
...
std::vector<double> ZOffsetcppMinimizer::FitParameters(I3VectorCCMPMTKey keys_to_fit, I3MapPMTKeyDouble LPmu, I3MapPMTKeyDouble LPsigma, I3MapPMTKeyDouble LPscale,
    ...
    std::vector<std::string> paramter_names = {"Rs", "tau_s", "tau_other", "norm", "uv_abs_1", "uv_abs_2"};
        ...
        for (size_t data_it = 0; data_it < all_constructorsAD.size(); data_it ++){
            ...
            for (size_t pmt_it = 0; pmt_it < keys_to_fit.size(); pmt_it ++){
                ...
                double llh = all_constructorsAD.at(data_it)->ComputeNLLH(key, params.at(0), Rt, params.at(1), tau_t, tau_rec, params.at(2), params.at(3), 
                                 time_offsets.at(data_it).at(key), const_offset, LPmu.at(key), LPsigma.at(key), LPscale.at(key), params.at(10 + pmt_it),
                                 {params.at(4), params.at(5)}, photons_per_mev, params.at(6 + data_it), n_sodium_events.at(data_it), light_profile_type, use_g4_yields).value();
                // now grab out data etc
