...
class NHit: public I3ConditionalModule {
    bool geo_seen;
...
NHit::NHit(const I3Context& context) : I3ConditionalModule(context),
...
void NHit::Geometry(I3FramePtr frame) {
...
void NHit::Physics(I3FramePtr frame) {
    if(not geo_seen) {
    ...
    std::vector<size_t> nhits(time_windows_.size(), 0);
    size_t nhit = 0;
    ...
    for(size_t time_bin = 0; time_bin < time_windows_.size(); ++time_bin) {
        frame->Put(output_prefix_ + "NHit" + std::to_string(int(time_windows_[time_bin])) + "NS", boost::make_shared<I3UInt32>((int32_t)nhits[time_bin]));
    }
